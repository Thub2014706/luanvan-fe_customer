...
export const showTimeByTheater = async (theater, typeSchedule) => {
    try {
        const response = await axios.get(
            `${process.env.REACT_APP_API_URL}/api/showtime/list-by-theater?theater=${theater}&typeSchedule=${typeSchedule}`,
        );
        return response.data;
    } catch (error) {
        console.log('loi', error);
...
export const filmByTheater = async (theater, date) => {
    try {
        const response = await axios.get(
            `${process.env.REACT_APP_API_URL}/api/showtime/film-by-theater?theater=${theater}&date=${date}`,
        );
        return response.data;
    } catch (error) {
        console.log('loi', error);
